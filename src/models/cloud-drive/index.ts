...
export default function useCloudDriveModel() {
  ...
  const updateUploadFile = useCallback(
    async (
      ...
      if (
        ['waiting', 'uploading', 'waitingForCompleted'].includes(uploadState)
      ) {
        newState = 'uploading';
  ...
  const updateDownloadFile = useCallback(
    (
      ...
      let newState = newFile.state;
      if (
        ['waiting', 'downloading', 'waitingForCompleted'].includes(
          downloadState,
        )
      ) {
  ...
  const autoDownload = useCallback(async () => {
    ...
    while (!!file) {
      ...
      try {
        const url =
          FILEOBJECT_DOWNLOAD_URL +
          '?fidlist=' +
          file.chunks.map((f) => f.id).join(',');
