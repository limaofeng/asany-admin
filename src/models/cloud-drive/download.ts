...
import { networkSpeed } from '@/metronic/utils';
...
async function downloadFetch(url: string, options?: DownloadFetchOptions) {
  ...
  if (brokenFile) {
    ...
    brokenFile.lastModified &&
      (_options.headers['If-Modified-Since'] = brokenFile.lastModified);
  ...
  const downloadFile = async () => {
    try {
      ...
      onDownloadProgress &&
        onDownloadProgress({
          loaded: Math.max(receivedLength, 1),
          total: contentLength,
        });
      ...
      while (true) {
        ...
        onDownloadProgress &&
          onDownloadProgress({ total: contentLength, loaded: receivedLength });
      }
...
export async function download(url: string, options?: DownloadOptions) {
  ...
  if (_options.fetchOptions && onDownloadProgress) {
    _options.fetchOptions.onDownloadProgress = debounce(
      onDownloadProgress,
      600,
      {
        leading: true,
...
type UseDownloadResult = [
  (
    url: string,
    overwrites?: DownloadOverwriteOptions,
  ) => Promise<DownloadFileData>,
  DownloadResult,
];
...
export const useDownload = ({
  ...
  const executeDownloadFile = useCallback(
