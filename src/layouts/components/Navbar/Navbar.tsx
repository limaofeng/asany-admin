...
function NavbarDescription(props: NavbarDescriptionProps) {
  const { className, children } = props;
  return (
    <div className={classnames('d-flex flex-wrap fw-bold mb-4 fs-5 text-gray-400', className)}>
...
function NavbarTitle(props: NavbarTitleProps) {
  const { className, children } = props;
  return <div className={classnames('d-flex align-items-center mb-1', className)}>{children}</div>;
}
...
function NavbarBody(props: NavbarBodyProps) {
  const { className, children } = props;
  return (
    <div className={classnames('d-flex flex-wrap justify-content-start', className)}>
      {children}
    </div>
  );
}
...
function NavbarFooter(props: NavbarFooterProps) {
  ...
  return (
    <>
      ...
      <div className={classnames('d-flex overflow-auto', className)}>{children}</div>
    </>
  );
}
...
function Navbar(props: NavbarProps) {
  const { cover, header, body, footer } = useMemo(() => {
    ...
    let coverNode = childs.find((item) => React.isValidElement(item) && item.type == NavbarCover);
    let title = childs.find((item) => React.isValidElement(item) && item.type == NavbarTitle);
    let description = childs.find(
      (item) => React.isValidElement(item) && item.type == NavbarDescription,
    );
    const toolbar = childs.find((item) => React.isValidElement(item) && item.type == NavbarToolbar);
    let bodyNode = childs.find((item) => React.isValidElement(item) && item.type == NavbarBody);
    ...
    const headerNode = <NavbarHeader title={title} description={description} toolbar={toolbar} />;
