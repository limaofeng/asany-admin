  ...
  return (
    ...
    >
      ...
      {closable && (
        <div
          onClick={onCancel}
          className="btn btn-icon btn-active-color-primary align-self-start"
        >
...
function Modal(props: ModalProps) {
  ...
  const _footer = useMemo(() => {
    if (props.footer === false || props.footer === null) {
      return undefined;
    }
    if (props.footer) {
      return props.footer;
  ...
  const { header, body, footer } = useMemo(() => {
    ...
    const bodyNode = childs.find(
      (item) => React.isValidElement(item) && item.type === ModalBody,
    );
    const newChildren = childs.filter(
      (item) => ![footerNode, headerNode].includes(item),
    );
    return {
      header: headerNode || (
      ...
      body: bodyNode || (
        <ModalBody className={props.bodyClassName}>{newChildren}</ModalBody>
      ),
      footer:
        footerNode ||
        (_footer && (
          <ModalFooter className={footerClassName}>{_footer}</ModalFooter>
        )),
    };
  }, [
    props.children,
