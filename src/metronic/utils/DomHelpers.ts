...
function getCSS(el: HTMLElement, styleProp: string) {
  ...
  const _styleProp = styleProp.replace(/([A-Z])/g, '-$1').toLowerCase();

  return defaultView.getComputedStyle(el, null).getPropertyValue(_styleProp);
}
...
function getElementActualCss(el: HTMLElement, prop: any, cache: boolean): any | undefined {
  let css = '';
...
function throttle(_timer: number | undefined, func: () => void, delay?: number) {
...
function slide(el: HTMLElement, dir: string, _speed: number, callback: any) {
  if (
    !el ||
...
function colorLighten(_1color: string, _1amount: number) {
  ...
  let color = _1color.indexOf('#') >= 0 ? _1color.substring(1, _1color.length) : _1color;
  const amount = parseInt(((255 * _1amount) / 100).toString());
...
function colorDarken(_1color: string, _1amount: number) {
