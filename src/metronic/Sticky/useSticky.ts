...
function useSticky(options: StickyOptions): [
  ...
  const _calculateHeight = useCallback(() => {
    ...
    if (KTUtil.css(element.current, 'border-bottom')!) {
      height =
        height + parseFloat(KTUtil.css(element.current, 'border-bottom')!);
    }
  ...
  useEffect(() => {
    state.current.height = _calculateHeight();
    state.current.width = KTUtil.css(element.current!, 'width')! as any;
