...
function CardToolbar(props: CardToolbarProps) {
  ...
  return (
    <div className={classnames('card-toolbar', className)}>{children}</div>
  );
}
...
function Card(
  props: CardProps & { to?: string; onClick?: () => void },
  ref: any,
) {
  const {
    as,
  ...
  const { extra, header, toolbar, body, footer, title } = useMemo(() => {
    ...
    const newChildren = childs.filter(
      (item) =>
        ![toolbarNode, footerNode, headerNode, titleNode].includes(item),
    );
    ...
    return {
      ...
      toolbar: toolbarNode,
      body: bodyNode || (
        <CardBody className={bodyClassName}>{newChildren}</CardBody>
      ),
      footer: footerNode,
...
interface CardWrapper
  extends React.ForwardRefExoticComponent<
    CardProps & React.RefAttributes<any | null>
  > {
