...
type RepeaterItemRenderer = (args: { remove: () => void; size: number }) => React.ReactNode;
...
function reducer(_state: FormRepeaterState, action: FormRepeaterAction) {
  ...
  if (action.type == FormRepeaterKind.REMOVE) {
    return { ..._state, items: _state.items.filter((item) => item.key !== action.payload) };
  }
  ...
  return (
    <div>
      <div className="form-group">
        ...
        >
          {items.map((item) => (
            <RepeaterItem key={item.key} size={items.length} value={item} remove={handleRemove}>
              {children}
            </RepeaterItem>
          ))}
        </div>
