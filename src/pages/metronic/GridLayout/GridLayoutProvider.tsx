...
import { assign, generateUUID, useDeepCompareEffect } from './utils';
import type {
  CompactType,
  IGridItem,
  IGridItemLayout,
...
function useStore(
  ...
  const [state, dispatch] = useReducer<React.ReducerWithoutAction<IGridLayoutState>>(
    ((_state: IGridLayoutState, action: SortableAction): IGridLayoutState => {
      ...
      if (action.type === SortableActionType.resize) {
        ...
        const { x, y, maxW = Infinity, minW = 0, maxH = Infinity, minH = 0 } = l;
        let { w, h } = calcWH(
          {
            cols,
            containerPadding,
        ...
        return update(_state, {
          layout: {
            $set: compact(layout, getCompactType(_state.verticalCompact, _state.compactType), cols),
          },
        });
      }
  ...
  const outside = _items.map(({ _originalSortable, _sortable, _rect, ...item }: any) => item);
  useDeepCompareEffect(() => {
    const _state = store.getState();
    const inside = _state.items.map(({ _originalSortable, _sortable, _rect, ...item }) => item);
  ...
  }, [_cols, _verticalCompact, _compactType, _rowHeight, _margin, _containerPadding]);
  return store;
}
