 ...
 */
export function getLayoutItem(layout: Layout, id: string): IGridItemLayout | undefined {
  for (let i = 0, len = layout.length; i < len; i++) {
...
export function getAllCollisions(layout: Layout, layoutItem: IGridItemLayout): IGridItemLayout[] {
...
export function getItemCoord(layout: React.RefObject<HTMLDivElement>, item: IGridItemInternalData) {
  const layoutRect = layout.current!.getBoundingClientRect();
  ...
  return {
    ...
    compare(other: ICoord): Relation {
      return other.direction({ y: top, x: left, h: itemRect.height, w: itemRect.width });
    },
  };
}
...
export const injectAnime = (props: any): AnimatedProps => {
  ...
  const anime = Object.keys(props)
    .filter((_key: string) => _key.startsWith('data-flip') || _key === 'data-portal-key')
    .reduce((data, name) => {
