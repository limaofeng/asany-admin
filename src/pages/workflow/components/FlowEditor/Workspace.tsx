...
function Workspace() {
  ...
  const sidebarMinWidth = useEditorSelector(
    (state) => state.ui.sidebar.minWidth,
  );
  const sidebarMinimizable = useEditorSelector(
    (state) => state.ui.sidebar.minimizable,
  );
  ...
  useEffect(() => {
    if (editorJson) {
      const flowData = flowableToReactflow(
        JSON.parse(editorJson) as ProcessDefinition,
      );
      setState(flowData);
  ...
  const minimizable = useEditorSelector(
    (state) => state.ui.sidebar.minimizable,
  );
