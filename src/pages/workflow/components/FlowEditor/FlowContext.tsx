...
export const useQuickControls = (): [
  ...
  return [
    ...
    {
      show: context.tools.showQuickControls,
      hide: context.tools.hideQuickControls,
    },
  ];
};
...
export const useNodesState = <NodeData = any,>(): [
  ...
  const handleSetNodes: Dispatch<SetStateAction<Node<NodeData>[]>> =
    useCallback(
      (s) => {
...
export const useEdgesState = <EdgeData = any,>(): [
  ...
  const handleSetEdges: Dispatch<SetStateAction<Edge<EdgeData>[]>> =
    useCallback(
      (callback) => {
...
export const FlowStateProvider = ({ children }: FlowStateProviderProps) => {
  const [
    state,
    setState,
    { undo, canUndo, redo, canRedo, reset, resetInitialState },
