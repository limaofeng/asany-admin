...
function Sidebar() {
  ...
  const match = useMatch({ path: '/mail/:folder', end: false });
  ...
  const mailboxes = useMemo(() => {
    if (!data?.mailUser || !mailboxesData?.mailboxes) {
      ...
      return {
        ...
        private: DEFAULT_MAILBOXES_ALL.filter(
          (item) => !['Outbox', 'INBOX'].includes(item.id),
        ).map((item) => ({
          ...item,
          title: item.name,
    ...
    const allMailboxes = mailboxesData.mailboxes
      .map((item) => {
        if (item.namespace === '#private') {
          const index = DEFAULT_MAILBOXES_ALL.findIndex(
            (m) => m.id === item.name,
          );
          return {
            ...item,
      ...
      .filter((item) => {
        ...
        return data?.mailUser?.settings?.mailboxes.includes(
  ...
  const unreadNumber = useMemo(
    () => countUnread?.inbox?.unread || 0,
    [countUnread?.inbox?.unread],
  );
  ...
  return (
    <AsideWorkspace
      width={280}
      collapsible={false}
      className="email-sidebar-aside"
    >
      <div className="email-sidebar">
        <Button
          as={Link}
          to="/mail/compose"
          className="email-compose text-uppercase"
        >
          写信
        </Button>
