...
function WebsiteList(props: WebsiteListProps) {
  ...
  return (
    <ContentWrapper header={{}} loading={loading} className="website-list" footer={false}>
        ...
        >
          {websites.map((item) => (
            <Col key={item.id} md={4}>
              <Card as={Link} to={`/websites/${item.id}`} className="website-card card-p-0">
                <Card.Body>
                  <Symbol
                    autoColor={false}
      ...
      <div className="create-new-website">
        <div className="d-flex flex-column">
          ...
          <span className="tw-text-ellipsis overflow-hidden">创建一个空白网站或从模板开始。</span>
        </div>
        <Row
            ...
            >
              <Card.Body>
                <div className="create-website-card-inner">
                  ...
                  <span className="create-website-card__blank_name text-gray-700">空白</span>
                </div>
              </Card.Body>
            </Card>
          </Col>
