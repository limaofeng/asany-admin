...
function StoreDetails() {
  ...
  const [loadStore, { data: viewData, loading: getLoading }] =
    useLandingStoreLazyQuery({
  ...
  const handleGeolocation = useCallback(async () => {
    ...
    const [longitude, latitude] =
      _data!.amapOpenAPI!.geocodes[0].location!.split(',');
    form.setFieldsValue({ location: { longitude, latitude } });
  }, [form, loadGeocode]);

  return (
    <ContentWrapper
      header={isNew ? { title: '新增门店' } : undefined}
      footer={false}
    >
      <Card className="mb-5 mb-xl-10">
        ...
        <Card.Body>
          ...
          <Button
            loading={getLoading || createSubmiting || updateSubmiting}
            onClick={handleSave}
          >
            {isNew ? <>添加门店</> : <>修改门店信息</>}
          </Button>
