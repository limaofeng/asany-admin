...
import Sortable, { dragPreview } from '@asany/sortable';
...
function ModelFieldsManagement(props: ModelFieldsManagementProps) {
  ...
  const { systemFields, customFields } = useMemo(() => {
    return model.fields.reduce(
      (fieldSets, field) => {
        if (field.system) {
          fieldSets.systemFields.push({
            id: field.id,
            data: field,
            type: 'system',
          });
        } else {
          fieldSets.customFields.push({
            id: field.id,
            data: field,
            type: 'custom',
          });
  ...
  const handleSort = useCallback(
    (value: ISortableItem[], event: SortableChangeEvent) => {
      console.log(value, event);
    },
    [],
  );

  const { data } = useModelFiledTypesQuery({
    ...
    <div className="model-fields-management">
      ...
      >
        ...
        <Sortable
          onChange={() => {}}
          draggable={false}
          className={classnames('system-fields', {
            'd-none': !state.includeSystemFields,
          })}
          tag="ul"
          items={systemFields}
        ...
        <div className="model-field-types-container">
          ...
          {fieldTypeGroups.map((g) => (
            <ModelFieldTypeGroup key={g.key} name={g.name}>
              {g.filedTypes.map((t) => (
                <ModelFieldType
                  onClick={handleCreateByFiledType}
                  key={t.id}
                  data={t}
                />
              ))}
