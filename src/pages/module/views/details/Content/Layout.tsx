...
function ContentLayout(props: ContentLayoutProps) {
  ...
  const handleViewClick = useCallback(
    (v: ModelView) => () => {
      history.push(`${baseUrl}/content/${v.model.id}/views/${v.id}`);
    },
    [baseUrl, history],
  );
  ...
  return (
    <>
      <SecondarySidebar loading={loading} className="module_secondary_sidebar" width={260}>
        <div className="h-100">
          ...
          >
            ...
            >
              ...
              <Menu.SubMenu key="custom-view" linkClassName="py-0" title="自定义视图">
              ...
              <Menu.SubMenu key="default-view" linkClassName="py-0" title="默认视图">
                {loading ? (
                  <Menu.Content contentClassName="custom-view-notes menu-section text-muted text-uppercase fs-8 ls-1">
                    加载中...
                  </Menu.Content>
                  ...
                  defaultViews.map((item) => (
                    <Menu.Item key={`view_${item.id}`} onClick={handleViewClick(item)}>
                      {item.name}
                    </Menu.Item>
                  ))
                )}
