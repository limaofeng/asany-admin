...
const CalendarListItem = React.forwardRef(function (
    ...
    >
      <Checkbox
        style={{
          ...
          borderColor: actived
            ? lightenColor(data.color!, 80)
            : darkenColor(data.color!),
        }}
        checked={checked}
...
function CalendarList(props: CalendarListProps) {
  ...
  const { data, refetch } = useCalendarsQuery({
  ...
  return (
    <Shortcuts
      ...
      name="CALENDAR_PREFERENCES"
    >
      <div className="calendar-list-notes">
        “{calendarSet?.name}”日历集的日历和任务列表。
      </div>
