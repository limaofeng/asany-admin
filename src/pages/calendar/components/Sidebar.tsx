...
function defaultEventWithDay(
  today: Moment,
  eventMap: Map<string, EventWithDay>,
) {
  // 添加最近七天的数据
  for (let i = 0; i < 7; i++) {
...
function Sidebar() {
  ...
  const fullCalendar = useModel(
    'calendar',
    (model) => model.state.fullCalendar,
  );
  ...
  const [
    ...
    { refetch: refetchCalendarEventsWithDays },
  ] = useCalendarEventsWithDaysLazyQuery({
  ...
  const gotoDate = useCallback((date: Date) => {
    ...
    const node = nodes.find((item) =>
      moment(item.dataset.date).isSame(moment(date), 'date'),
    );
    if (!node) {
      return;
    }
  ...
  const handleScroll = useCallback(() => {
    ...
    state.current.eventsWithDays = state.current.eventsWithDays.filter(
    ...
    setSelectedDay(
      (state.current.selectedDay = moment(node.dataset.date).toDate()),
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [setSelectedDay]);
  ...
  const handleSuccess = useCallback(
    (event: CalendarEvent) => {
      const mainCalendar = document.getElementsByClassName(
        'main-calendar',
      )[0] as any;
      mainCalendar?.focus();
      setNewed('none');
  ...
  return (
    <AsideWorkspace
      ...
      collapsible={false}
      resizeable={true}
      className={classnames('calendar-sidebar-bg', {
        'state-new-event': isNew,
      })}
    >
      <div className="calendar-sidebar">
        ...
        <NewCalendarEvent
          ...
          calendarSets={
            (calendarSetData?.calendarSets as any) || DEFAULT_CALENDARSETS
          }
        />
        <Calendar
          locale={zh_CN as any}
        ...
        <div className="calendar-day-list">
          ...
          >
            {eventsWithDays.map(
              ({ key, date, isToDay, events, isSelected, isTemporary }) => (
                <div
