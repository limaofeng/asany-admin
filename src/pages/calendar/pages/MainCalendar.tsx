...
function MainCalendar() {
  ...
  const handleNavLink = useCallback((e: any) => {
    fullCalendar.current
      ?.getApi()
      .changeView('timeGridDay', new Date(e.target.dataset.date));
  }, []);
          ...
          views={{
            dayGridMonth: {
              dayCellClassNames(item) {
                const _classnames = [];
                if (item.isOther) {
                  const prevmont = moment(item.view.currentStart).subtract(
                    1,
                    'days',
                  );
                  const nextmonth = moment(item.view.currentEnd);

                  const prevdays = moment(item.date).diff(
                    moment(prevmont),
                    'days',
                  );
                  const nextdays = moment(item.date).diff(
                    moment(nextmonth),
                    'days',
                  );
            ...
            dayGridYear: {
              dayCellClassNames(item) {
                ...
                if (
                  !item.isOther &&
                  moment(item.date).isSame(moment(selectedDay), 'date')
                ) {
                  _classnames.push('fc-day-selected');
                }
                return _classnames;
