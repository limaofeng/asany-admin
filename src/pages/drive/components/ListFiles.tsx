...
function ListFiles(props: ListFilesProps) {
  ...
  const currentFolder = useMemo(() => {
    ...
    return (temp.current.currentFolder =
      loadFolderResult?.currentFolder as FileObject);
    ...
    async (item: FileObject) => {
      if (toolbar === 'trash') {
        await Modal.confirm({
          ...
          content: (
            <>
              要查看该文件{item.isDirectory && '夹'}，您需要将其从回收站还原。
            </>
          ),
          okText: '还原',
        });
      ...
      if (item.isDirectory) {
        ...
        if (!item.isRootFolder) {
          const _paths: FileObject[] = [
            ...(generatePaths(
              temp.current.rootFolder,
              temp.current.currentFolder,
            ) as any),
            _currentFolder,
          ];
  ...
  const handleDelete = useCallback(async () => {
    ...
    const { data: rdata } = await moveToTrash({
  ...
  const handleRestore = useCallback(async () => {
    ...
    const { data: rdata } = await restoreFiles({
      variables: { ids: selectedKeys },
    });
    setSelectedKeys([]);
    refetchWithRemoveForObjects(rdata?.files as any, (l, r) => l.id === r.id);
          ...
          <Card.Toolbar>
            <div
              className={classnames('d-flex justify-content-end', {
                'file-action-group gap-2': ['row', 'multi-row'].includes(
                  row_selection_state,
                ),
              })}
            >
