...
type ArticleChannelNewProps = RouteComponentProps<
  { id: string },
  any,
  { rootChannelId: string; channels: ArticleChannel[] }
>;
...
function ArticleChannelNew(props: ArticleChannelNewProps) {
  const {
    ...
    location: {
      state: { rootChannelId, channels },
    },
  } = props;

  console.log('rootChannelId', rootChannelId);
  ...
  const channelsTreeData = useMemo(() => {
    if (!channels || !channels.length) {
      return [];
    }
    return tree<any, { value: string; title: string }>(
      channels.map((item: any) => ({ ...item, value: item.id, title: item.name })),
      {
        idKey: 'id',
  ...
  console.log('ArticleNew -> ', props, channels, channelsTreeData);
