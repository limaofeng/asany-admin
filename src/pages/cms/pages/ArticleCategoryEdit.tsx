...
function ArticleChannelSettings(props: ArticleChannelSettingsProps) {
  ...
  const handleFinish = useCallback(
    async (values: any) => {
      await updateCategory({ variables: { id: match.params.id, input: values } });
    ...
    [match.params.id, updateCategory],
  );
  ...
  useEffect(() => {
    if (!data?.category) {
      form.setFieldsValue({ storeTemplate: '1' });
      return;
    }
    form.setFieldsValue({
      name: data.category.name,
      parent: data.category.parent?.id == rootCategoryId ? null : data.category.parent?.id,
    });
  }, [data?.category, form, rootCategoryId]);
  ...
  return (
    ...
    >
      <Form onFinish={handleFinish} form={form} className="form d-flex flex-column flex-lg-row">
