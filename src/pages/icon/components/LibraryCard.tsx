...
function LibraryCard({ library, onStatusChange }: LibraryCardProps) {
  ...
  const [deleteLibrary, { loading: _deleting }] =
    useDeleteIconLibraryMutation();
  const [updateLibrary, { loading: _updating }] =
    useUpdateIconLibraryMutation();

  const handleClick = useCallback(() => {
    navigate('/icon-libraries/' + library.id);
  }, [history, library.id]);
    ...
    <Card shadow="sm" className="library-container">
      <Card.Body>
        <Spin spinning={state.deleting}>
          <div
            onClick={handleClick}
            className="library-image-wrapper empty-library"
          />
        </Spin>
      </Card.Body>
      <Card.Footer>
