...
const SwitchMsgType: FC<SwitchMsgTypeProps> = ({ msg, audio, curCve, selfID }) => {
      ...
      if (member) {
        mstr = mstr.replaceAll(
          ...
          `<span onclick='userClick("${member.userID.replace(
            '.',
            '-',
          )}")' style="color:#428be5;cursor: pointer;"> @${member.nickname} </span>`,
        );
      } else {
        mstr = mstr.replaceAll(
          a,
  ...
  const parseQute = (quMsg: MessageItem) => {
    switch (quMsg.contentType as number) {
      case messageTypes.TEXTMESSAGE:
        ...
        return <div className="content" dangerouslySetInnerHTML={{ __html: parsedMsg }} />;
      ...
      case messageTypes.ATTEXTMESSAGE:
        ...
        return (
          <div style={sty} className={`chat_bg_msg_content_text ${!isSingle ? 'nick_magin' : ''}`}>
            <div style={{ display: 'inline-block' }} dangerouslySetInnerHTML={{ __html: atMsg }} />
            {timeTip()}
          </div>
        );
      case messageTypes.PICTUREMESSAGE:
        ...
        return (
          ...
          >
            <div className="qute_content">
              <div>{`${'Reply' + msg.quoteElem.quoteMessage.senderNickname}:`}</div>
              {parseQute(quMsg)}
            </div>
