              ...
              <div className="position-relative">
                <div className="overlay overlay-show">
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "url('/assets/media/background/mage_icon.png')",
                    }}
                  />
                  <div
                    className="overlay-layer rounded"
                    style={{ opacity: 0.4 }}
                  />
                </div>
                <div className="position-absolute text-white mb-4 ms-4 bottom-0">
                  <span className="text-white fs-4 fw-bolder mb-1">
                    {data.title}
                  </span>
...
function Components() {
  ...
  const libraryId = app.dependencies!.find(
    (item) => item.name === 'component.library',
  )!.value;
  ...
  const handleEdit = useCallback(
    (_data: Component) => {
      setState((prevState) => ({
        ...prevState,
        visible: true,
        component: { ..._data, libraryId } as any,
      }));
    },
    [libraryId],
  );
