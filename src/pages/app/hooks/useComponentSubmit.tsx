...
import { LoadComponentsDocument, useCreateComponentMutation, useUpdateComponentMutation } from '.';
...
function useComponentSubmit(
    ...
    if (!!data?.id) {
      ...
      return async () => {
        ...
        try {
          ...
          const { data: rdata } = await Toast.promise(
            delay(
              createComponent({ variables: { input: { ...input, libraryId: data.libraryId } } }),
              350,
            ),
            {
              pending: '提交中...',
  ...
  }, [createComponent, data?.libraryId, data?.id, form, setData, updateComponent]);
