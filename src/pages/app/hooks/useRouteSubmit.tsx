...
import { LoadRoutesDocument, useCreateRouteMutation, useUpdateRouteMutation } from '../hooks';
...
function useRouteSubmit(
    ...
    if (!!data?.id) {
      return async () => {
        ...
        try {
          ...
          const { data: rdata } = await Toast.promise(
            delay(updateRoute({ variables: { id: data.id, input: { ...input } } }), 350),
            {
              pending: '提交中...',
              success: {
                render: () => (
      ...
      return async () => {
        ...
        try {
          ...
          const { data: rdata } = await Toast.promise(
            delay(
              createRoute({ variables: { input: { ...input, application: data.application.id } } }),
              350,
            ),
            {
              pending: '提交中...',
  ...
  }, [createRoute, data?.application?.id, data?.id, form, setData, updateRoute]);
